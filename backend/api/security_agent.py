...
Now includes Virtual Cybersecurity Sandbox for live container testing
"""
...
# Virtual Cybersecurity Sandbox imports
import sys
import os
...
# Add scripts directory for VCDB case study analysis
scripts_dir = os.path.join(os.path.dirname(backend_dir), 'scripts')
sys.path.insert(0, scripts_dir)

try:
    import sandbox
    from sandbox.api import setup_sandbox_api
    SANDBOX_AVAILABLE = True
    print("✅ Virtual Cybersecurity Sandbox loaded successfully")
except ImportError as e:
    print(f"⚠️  Virtual Cybersecurity Sandbox not available: {e}")
    SANDBOX_AVAILABLE = False

# Case Study Analysis imports
try:
    from agent_vcdb_integration import analyze_architecture, search_similar_cases
    ...
    print("✅ VCDB Case Study Analysis loaded successfully")
except ImportError as e:
    print(f"⚠️  VCDB Case Study Analysis not available: {e}")
...
app = FastAPI(title="InsightX Security Agent API with Virtual Sandbox", version="1.0.0")
...
# Setup Virtual Cybersecurity Sandbox API routes
if SANDBOX_AVAILABLE:
    try:
        sandbox_api = setup_sandbox_api(app)
        print("🚀 Virtual Cybersecurity Sandbox API initialized")
        
        # Add sandbox status endpoint to main API
        @app.get("/api/sandbox-status")
        async def get_sandbox_status_info():
            """Check if Virtual Cybersecurity Sandbox is available"""
            return {
                "sandbox_available": True,
                "features": [
        ...
        print(f"❌ Failed to initialize Virtual Cybersecurity Sandbox: {e}")
        SANDBOX_AVAILABLE = False

# Add fallback endpoint if sandbox not available
if not SANDBOX_AVAILABLE:
    @app.get("/api/sandbox-status")
    async def get_sandbox_status_info():
        """Sandbox not available status"""
        return {
            "sandbox_available": False,
            "message": "Virtual Cybersecurity Sandbox requires Docker and additional dependencies",
            "status": "unavailable"
        }
...
class CaseStudyAnalysisRequest(BaseModel):
    architecture: Architecture
    attack: Optional[ConfiguredAttack] = None
...
def analyze_case_studies(architecture: Dict, attack: Optional[Dict] = None) -> Dict:
    """
    Analyze architecture against historical case studies using VCDB data
    """
    print("🔍 Analyzing architecture against historical case studies...")
    
    if not CASE_STUDY_AVAILABLE:
        # Mock case study analysis for demo purposes
        return create_mock_case_study_analysis(architecture, attack)
    
    try:
        # Convert architecture to searchable format
        components = []
        for node in architecture.get('nodes', []):
            name = node.get('name', '')
            component_type = node.get('properties', {}).get('component_type', '')
            if name and component_type:
                components.append({"name": f"{name} ({component_type})"})
        
        # Add attack information if available
        potential_attacks = []
        if attack:
            potential_attacks = [attack.get('attack_name', ''), attack.get('category', '')]
            potential_attacks = [a for a in potential_attacks if a]
        
        # Create searchable architecture object
        search_arch = {
            "components": components,
            "potential_attacks": potential_attacks
        }
        
        # Use existing VCDB analysis
        result = analyze_architecture(search_arch, top_k=10)
        
        # Enhance with AI analysis
        enhanced_analysis = enhance_case_study_analysis(result, architecture, attack)
        
        return enhanced_analysis
        
    except Exception as e:
        print(f"⚠️ Error in case study analysis: {e}")
        return create_mock_case_study_analysis(architecture, attack)

def create_mock_case_study_analysis(architecture: Dict, attack: Optional[Dict] = None) -> Dict:
    """
    Create mock case study analysis for demo purposes
    """
    print("🎭 Using mock case study data for demonstration")
    
    # Extract architecture info
    nodes = architecture.get('nodes', [])
    has_web_server = any('web_server' in str(node.get('properties', {}).get('component_type', '')).lower() for node in nodes)
    has_database = any('database' in str(node.get('properties', {}).get('component_type', '')).lower() for node in nodes)
    has_firewall = any('firewall' in str(node.get('properties', {}).get('component_type', '')).lower() for node in nodes)
    
    # Mock similar cases based on architecture
    similar_cases = []
    
    if has_web_server and has_database:
        similar_cases.append({
            "incident_id": "VCDB-2023-0542",
            "industry": "Retail",
            "country": "United States",
            "actor": "External",
            "action": "SQL Injection attack leading to data breach",
            "asset": "Web application, Database server",
            "impact_overall_rating": "Major",
            "summary": "Attackers exploited SQL injection vulnerability in e-commerce platform, accessing customer payment data",
            "distance": 0.15,
            "year": "2023"
        })
    ...
    if not has_firewall:
        similar_cases.append({
            "incident_id": "VCDB-2022-0891",
            "industry": "Healthcare",
            "country": "Canada", 
            "actor": "External",
            "action": "Ransomware attack via exposed RDP",
            "asset": "Server infrastructure",
            "impact_overall_rating": "Severe",
            "summary": "Hospital systems compromised through exposed Remote Desktop Protocol without firewall protection",
            "distance": 0.22,
            "year": "2022"
        })
    
    if attack and 'injection' in attack.get('attack_name', '').lower():
        similar_cases.append({
            "incident_id": "VCDB-2023-1205",
            "industry": "Financial",
            "country": "United Kingdom",
            "actor": "External",
            "action": "Code injection via web application",
            "asset": "Banking application servers",
            "impact_overall_rating": "Major",
            "summary": "Financial institution suffered code injection attack, compromising account management system",
            "distance": 0.08,
            "year": "2023"
        })
    ...
    similar_cases.extend([
        {
            "incident_id": "VCDB-2022-0334",
            "industry": "Technology",
            "country": "Germany",
            "actor": "External",
            "action": "Phishing leading to credential compromise",
            "asset": "Employee workstations, Cloud services",
            "impact_overall_rating": "Moderate",
            "summary": "Tech company employees targeted by sophisticated phishing campaign",
            "distance": 0.35,
            "year": "2022"
        },
        {
            "incident_id": "VCDB-2021-0789",
            ...
            "impact_overall_rating": "Severe",
            "summary": "Manufacturing facility compromised through third-party software vulnerability",
            "distance": 0.45,
            "year": "2021"
        }
    ])
    ...
    if not similar_cases:
        similar_cases.append({
            "incident_id": "VCDB-2023-0001",
            "industry": "General",
            "country": "Unknown",
            "actor": "External",
            "action": "Generic cyber attack",
            "asset": "IT Infrastructure",
            "impact_overall_rating": "Moderate",
            "summary": "General cybersecurity incident affecting IT infrastructure",
            "distance": 0.50,
            "year": "2023"
        })
    
    # Create query text
    component_names = [node.get('name', '') for node in nodes]
    attack_name = attack.get('attack_name', 'Unknown') if attack else 'General security analysis'
    query = f"Architecture with {', '.join(component_names[:3])} facing potential {attack_name}"
    
    return {
        "similar_cases": similar_cases[:5],  # Limit to top 5
        "analysis": {
            "risk_patterns": [
                f"Architecture lacks firewall protection" if not has_firewall else "Firewall protection detected",
                f"Web-database architecture common attack target" if has_web_server and has_database else "",
                f"Database exposed without proper segmentation" if has_database and not has_firewall else ""
            ],
            "recommendations": [
                ...
                "Establish incident response procedures based on similar cases",
                "Regular security assessments and penetration testing"
            ],
            "confidence_score": 0.85 if similar_cases else 0.5
        },
        "meta": {
            "total_cases_analyzed": len(similar_cases),
            "search_strategy": "Mock data analysis",
            "vcdb_available": CASE_STUDY_AVAILABLE
        }
    }

def enhance_case_study_analysis(vcdb_result: Dict, architecture: Dict, attack: Optional[Dict]) -> Dict:
    """
    Enhance VCDB results with AI analysis and insights
    """
    print("🤖 Enhancing case study analysis with AI insights...")
    
    # Prepare data for AI analysis
    similar_cases = vcdb_result.get('similar_cases', [])
    ...
    for i, case in enumerate(similar_cases[:5], 1):
        cases_summary.append(f"""
Case {i}:
- Industry: {case.get('industry', 'Unknown')}
- Attack Vector: {case.get('action', 'Unknown')}
- Targeted Assets: {case.get('asset', 'Unknown')}
- Impact: {case.get('impact_overall_rating', 'Unknown')}
- Summary: {case.get('summary', 'No summary available')}
- Similarity Score: {1.0 - case.get('distance', 1.0):.2f}
        """)
    
    prompt = f"""You are a cybersecurity expert analyzing historical case studies to provide insights.

CURRENT ARCHITECTURE:
{json.dumps(architecture, indent=2)}

CURRENT ATTACK SCENARIO:
{json.dumps(attack, indent=2) if attack else "No specific attack provided"}

TOP 5 SIMILAR HISTORICAL CASES:
{''.join(cases_summary)}

TASK: Provide comprehensive analysis in JSON format:

{{
  "analysis": {{
    "risk_patterns": ["<specific risks identified from cases>"],
    "recommendations": ["<actionable security recommendations>"],
    "confidence_score": <0.0-1.0 based on case similarity and quantity>
  }},
  "meta": {{
    "total_cases_analyzed": {len(similar_cases)},
    "search_strategy": "AI-enhanced historical analysis",
    "vcdb_available": true
  }},
  "additional_insights": {{
    "top_attack_vectors": ["<most common attack types from cases>"],
    "industry_patterns": ["<common industries affected>"],
    "common_vulnerabilities": ["<vulnerabilities that appear in multiple cases>"],
    "risk_level": "<assess: CRITICAL/HIGH/MEDIUM/LOW>",
    "key_insights": ["<3-5 key insights from case analysis>"]
  }}
}}

Analyze the cases and provide real insights based on the historical data."""

    try:
        messages = [{"role": "user", "content": prompt}]
        ai_response = agent.model.invoke(messages)
        enhanced_analysis = extract_json_from_response(ai_response.content)
        
        # Merge with original VCDB result
        result = vcdb_result.copy()
        result.update(enhanced_analysis)
        
        print("✅ Enhanced case study analysis completed")
        return result
        
    except Exception as e:
        print(f"⚠️ Error in AI enhancement: {e}")
        # Return basic analysis in expected format
        return {
            **vcdb_result,
            "analysis": {
                "risk_patterns": [
                    f"Found {len(similar_cases)} similar historical incidents",
                    "Analysis requires AI enhancement for detailed patterns"
                ],
                "recommendations": [
                    "Implement security monitoring based on historical cases",
                    "Review similar incidents for prevention strategies"
                ],
                "confidence_score": 0.7 if similar_cases else 0.3
            },
            "meta": {
                "total_cases_analyzed": len(similar_cases),
                "search_strategy": "Basic pattern matching",
                "vcdb_available": True
            }
        }
...
COMPONENT TYPES AVAILABLE (Use EXACTLY these component_type names):

COMPUTE & APPLICATION:
- web_server, application_server, serverless_function, api_gateway

DATA & STORAGE:  
- database, cache_server, file_storage, object_storage, managed_database, data_warehouse

NETWORK & INFRASTRUCTURE:
- load_balancer, cdn, proxy_server, nat_gateway, router, switch, dns_server, bastion_host

SECURITY:
- firewall, waf, ids_ips, vpn_gateway, siem, identity_provider, secrets_manager, dlp_system, certificate_authority, vulnerability_scanner, endpoint_protection, penetration_testing

CLOUD SERVICES:
- kubernetes_cluster, virtual_machine, auto_scaling_group, cloud_function, queue_service, cloud_storage_gateway

USER ACCESS & DEVICES:
- user_device, admin_workstation, mobile_device_management, remote_access_portal, privileged_access_management, wireless_access_point

EXTERNAL & THIRD-PARTY:
- third_party_api, payment_gateway, cdn_provider, cloud_provider, saas_application, dns_provider

SPECIALIZED & INFRASTRUCTURE:
- message_queue, email_server, backup_server, monitoring_server, container_registry, cicd_server, iot_device, blockchain_node, ml_pipeline

IMPORTANT: You MUST use the exact component_type names listed above. Do NOT create new component names or modify these names.

CRITICAL VALIDATION RULES:
1. Every "component_type" field MUST exactly match one of the component types above
2. NO variations, abbreviations, or alternative spellings allowed
3. NO adding suffixes like "_service", "_system", or "_component" 
4. NO pluralization (e.g., use "database" not "databases")
5. Use underscores, not hyphens (e.g., "web_server" not "web-server")

EXAMPLES OF CORRECT vs WRONG component types:
✅ CORRECT: "application_server"  
❌ WRONG: "app_server", "application", "app_service", "application_service"

✅ CORRECT: "user_device"
❌ WRONG: "user_workstation", "user_computer", "client_device", "end_user_device"

✅ CORRECT: "identity_provider" 
❌ WRONG: "authentication_service", "auth_service", "login_service", "identity_service"

✅ CORRECT: "monitoring_server"
❌ WRONG: "monitoring_service", "monitoring_system", "log_aggregator", "metrics_server"

✅ CORRECT: "message_queue"
❌ WRONG: "messaging_queue", "queue_service", "message_broker", "event_queue"

DOUBLE-CHECK: Before returning your JSON, verify that EVERY component_type field contains an EXACT match from the component types list above.
...
6. CRITICAL: Use ONLY the exact component_type names listed above
...
{{
  ...
  "nodes": [
    {{
      ...
      "name": "<descriptive name for this component>",
      "properties": {{
        "component_type": "<MUST be EXACTLY one of the component types listed above - NO VARIATIONS>",
...
- Include typical flow: users → load balancer → web servers → application servers → databases
- Add firewall/WAF if security is mentioned or for production systems
- Add caching (cache_server) if performance is mentioned
- Add monitoring (monitoring_server) for production systems
...
1. Use "application_server" instead of "app_server"
2. Use "user_device" instead of "user_workstation" 
3. Use "identity_provider" instead of "authentication_service"
4. Use "monitoring_server" instead of "monitoring_service"
5. Use "queue_service" or "message_queue" for messaging
6. For containers use "kubernetes_cluster" or "container_registry"
7. NEVER invent new component types - stick to the exact list provided

VALIDATION: Before finalizing, double-check that every "component_type" value in your JSON matches EXACTLY one of the component types in the list above.
...
Analyze the user's prompt and generate a REAL architecture based on their needs."""
        ...
        try:
            ...
            # Validate component types against componentRegistry.ts
            valid_component_types = {
                "web_server", "application_server", "database", "api_gateway", "cache_server", "message_queue", 
                "file_storage", "dns_server", "container_registry", "cicd_server", "email_server", "backup_server", 
                "monitoring_server", "serverless_function", "firewall", "load_balancer", "vpn_gateway", "proxy_server", 
                "nat_gateway", "router", "switch", "cdn", "bastion_host", "ids_ips", "waf", "siem", "identity_provider", 
                "secrets_manager", "dlp_system", "certificate_authority", "vulnerability_scanner", "endpoint_protection", 
                "penetration_testing", "kubernetes_cluster", "virtual_machine", "object_storage", "auto_scaling_group", 
                "managed_database", "cloud_function", "queue_service", "cloud_storage_gateway", "user_device", 
                "admin_workstation", "mobile_device_management", "remote_access_portal", "privileged_access_management", 
                "wireless_access_point", "third_party_api", "payment_gateway", "cdn_provider", "cloud_provider", 
                "saas_application", "dns_provider", "iot_device", "data_warehouse", "blockchain_node", "ml_pipeline"
            }
            
            # Validate and fix component types
            invalid_components = []
            fixes_applied = []
            ...
            for node in architecture.get('nodes', []):
                ...
                if component_type not in valid_component_types:
                    invalid_components.append({
                        'id': node.get('id'),
                        'name': node.get('name'),
                        'invalid_type': component_type
                    })
                    
                    # Comprehensive auto-fix mapping
                    type_mapping = {
                        # Application/App variations
                        'app_server': 'application_server',
                        'application': 'application_server',
                        'app_service': 'application_server',
                        'application_service': 'application_server',
                        'microservice': 'application_server',
                        'container': 'application_server',
                        'service': 'application_server',
                        
                        # User device variations  
                        'user_workstation': 'user_device',
                        'user': 'user_device',
                        'client_device': 'user_device',
                        'end_user_device': 'user_device',
                        'user_computer': 'user_device',
                        'workstation': 'user_device',
                        
                        # Authentication variations
                        'authentication_service': 'identity_provider',
                        'auth_service': 'identity_provider',
                        'login_service': 'identity_provider',
                        'identity_service': 'identity_provider',
                        'auth_server': 'identity_provider',
                        'sso_provider': 'identity_provider',
                        
                        # Monitoring variations
                        'monitoring_service': 'monitoring_server',
                        'monitoring': 'monitoring_server',
                        'log_aggregator': 'monitoring_server',
                        'logging_server': 'monitoring_server',
                        'metrics_server': 'monitoring_server',
                        'monitoring_system': 'monitoring_server',
                        
                        # Queue/Messaging variations
                        'messaging_queue': 'message_queue',
                        'queue_service': 'message_queue',
                        'message_broker': 'message_queue',
                        'event_queue': 'message_queue',
                        'messaging_service': 'message_queue',
                        ...
                        'redis_server': 'cache_server',
                        'memcached_server': 'cache_server',
                        
                        # Storage variations
                        'storage': 'file_storage',
                        'file_server': 'file_storage',
                        'storage_server': 'file_storage',
                        
                        # Web server variations
                        'web': 'web_server',
                        ...
                        'security_gateway': 'firewall',
                        'network_firewall': 'firewall',
                        
                        # API variations
                        'api': 'api_gateway',
                        'api_server': 'api_gateway',
                        'gateway': 'api_gateway',
                        ...
                        'backup_system': 'backup_server',
                        
                        # Common plurals and hyphenated versions
                        'databases': 'database',
                        'web-server': 'web_server',
                        'app-server': 'application_server',
                    ...
                    if component_type.lower() in type_mapping:
                        new_type = type_mapping[component_type.lower()]
                        node['properties']['component_type'] = new_type
                        fixes_applied.append(f"{original_type} → {new_type}")
                        fixed = True
                    elif component_type.replace('-', '_') in valid_component_types:
                        # Fix hyphen to underscore
                        new_type = component_type.replace('-', '_')
                        node['properties']['component_type'] = new_type
                        fixes_applied.append(f"{original_type} → {new_type}")
                        fixed = True
                    elif component_type.rstrip('s') in valid_component_types:
                        # Remove plural 's'
                        new_type = component_type.rstrip('s')
                        node['properties']['component_type'] = new_type
                        fixes_applied.append(f"{original_type} → {new_type}")
                        fixed = True
                    
                    if not fixed:
                        print(f"⚠️ Could not fix invalid component type: {component_type} in node {node.get('id')}")
            
            if fixes_applied:
                print(f"🔧 Applied {len(fixes_applied)} component type fixes:")
                for fix in fixes_applied:
                    print(f"   - {fix}")
            
            # Check for remaining invalid components
            remaining_invalid = [comp for comp in invalid_components 
                               if comp['invalid_type'] not in [fix.split(' → ')[0] for fix in fixes_applied]]
            
            if remaining_invalid:
                print(f"❌ Warning: {len(remaining_invalid)} component types could not be fixed:")
                for comp in remaining_invalid:
                    print(f"   - {comp['id']}: '{comp['invalid_type']}' in {comp['name']}")
                    print(f"     Available types: {', '.join(sorted(valid_component_types))}")
                
                # Force replace with closest match
                for comp_info in remaining_invalid:
                    node_id = comp_info['id']
                    invalid_type = comp_info['invalid_type']
                    
                    # Find the node and replace with a sensible default
                    for node in architecture.get('nodes', []):
                        if node.get('id') == node_id:
                            # Try to guess based on keywords
                            if any(keyword in invalid_type.lower() for keyword in ['web', 'http', 'front']):
                                node['properties']['component_type'] = 'web_server'
                                print(f"🔧 Forced fix: {invalid_type} → web_server")
                            elif any(keyword in invalid_type.lower() for keyword in ['app', 'service', 'api', 'business']):
                                node['properties']['component_type'] = 'application_server'
                                print(f"🔧 Forced fix: {invalid_type} → application_server")
                            elif any(keyword in invalid_type.lower() for keyword in ['data', 'db', 'sql', 'mongo']):
                                node['properties']['component_type'] = 'database'
                                print(f"🔧 Forced fix: {invalid_type} → database")
                            elif any(keyword in invalid_type.lower() for keyword in ['user', 'client', 'device']):
                                node['properties']['component_type'] = 'user_device'
                                print(f"🔧 Forced fix: {invalid_type} → user_device")
                            elif any(keyword in invalid_type.lower() for keyword in ['security', 'fire', 'protect']):
                                node['properties']['component_type'] = 'firewall'
                                print(f"🔧 Forced fix: {invalid_type} → firewall")
                            else:
                                # Default fallback
                                node['properties']['component_type'] = 'application_server'
                                print(f"🔧 Default fix: {invalid_type} → application_server")
                            break
            
            # Final validation - ensure ALL components are now valid
            final_invalid = []
            for node in architecture.get('nodes', []):
                component_type = node.get('properties', {}).get('component_type', '')
                if component_type not in valid_component_types:
                    final_invalid.append(f"{node.get('id')}: {component_type}")
            
            if final_invalid:
                error_msg = f"Invalid component types remain after auto-fix: {', '.join(final_invalid)}"
                print(f"❌ {error_msg}")
                raise ValueError(error_msg)
            
            print(f"✅ All {len(architecture.get('nodes', []))} components validated successfully")
...
@app.post("/api/analyze-case-studies")
async def analyze_case_studies_endpoint(request: CaseStudyAnalysisRequest):
    """
    Analyze architecture against historical cybersecurity case studies from VCDB
    Provides insights from real-world incidents and lessons learned
    """
    try:
        print(f"\n🔍 Starting case study analysis...")
        print(f"📊 Architecture: {request.architecture.metadata.get('company_name', 'Unknown')}")
        if request.attack:
            print(f"⚔️  Attack: {request.attack.attack_name}")
        
        # Perform case study analysis
        analysis_result = analyze_case_studies(
            request.architecture.model_dump(),
            request.attack.model_dump() if request.attack else None
        )
        
        # Format response
        response = {
            "analysis_id": f"case_study_{int(datetime.now().timestamp())}",
            "analysis_timestamp": datetime.now().isoformat(),
            "architecture_info": {
                "company_name": request.architecture.metadata.get("company_name", "Unknown"),
                "nodes_count": len(request.architecture.nodes),
                "connections_count": len(request.architecture.connections)
            },
            "attack_info": {
                "attack_name": request.attack.attack_name if request.attack else "General Analysis",
                "category": request.attack.category if request.attack else "N/A"
            },
            "case_study_results": analysis_result,
            "status": "completed"
        }
        
        print(f"✅ Case study analysis complete - Found {len(analysis_result.get('similar_cases', []))} similar cases")
        return response
        
    except Exception as e:
        print(f"❌ Error in case study analysis: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/enhanced-validation")
async def enhanced_validation_with_case_studies(request: ValidationRequest):
    """
    Enhanced attack validation that includes case study analysis
    Combines traditional validation with historical incident insights
    """
    try:
        print(f"\n🎯 Enhanced validation: {request.attack.attack_id}")
        
        # Step 1: Traditional validation
        print("🔍 Step 1: Traditional attack validation...")
        initial_state: AgentState = {
            "attack": request.attack.dict(),
            "architecture": request.architecture.dict(),
            "validation_result": {},
        ...
        validation_result = agent.validator_graph.invoke(initial_state)
        
        # Step 2: Case study analysis
        print("📚 Step 2: Historical case study analysis...")
        case_study_analysis = analyze_case_studies(
            request.architecture.dict(),
            request.attack.dict()
        )
        
        # Step 3: Combine insights
        print("🔗 Step 3: Combining insights...")
        enhanced_response = {
            "is_valid": validation_result.get("validation_result", {}).get("is_valid", True),
            "attack_id": request.attack.attack_id,
            "validation_timestamp": datetime.now().isoformat(),
            "traditional_analysis": {
                "missing_components": validation_result.get("validation_result", {}).get("missing_components", []),
                "security_analysis": validation_result.get("validation_result", {}).get("security_analysis", {}),
                "can_proceed": validation_result.get("validation_result", {}).get("can_proceed", True)
            },
            "historical_insights": {
                "similar_cases_found": len(case_study_analysis.get("similar_cases", [])),
                "avg_similarity": sum(1.0 - case['distance'] for case in case_study_analysis.get("similar_cases", [])) / max(1, len(case_study_analysis.get("similar_cases", []))),
                "lessons_learned": case_study_analysis.get("analysis", {}).get("recommendations", []),
                "industry_patterns": list(set([case.get('industry', 'Unknown') for case in case_study_analysis.get("similar_cases", [])])),
                "risk_assessment": "HIGH" if case_study_analysis.get("meta", {}).get("total_cases_analyzed", 0) > 3 else "MEDIUM"
            },
            "combined_recommendations": {
                "immediate_actions": case_study_analysis.get("analysis", {}).get("recommendations", []),
                "based_on_history": True,
                "confidence_level": "high" if len(case_study_analysis.get("similar_cases", [])) > 3 else "medium"
            },
            "case_study_details": case_study_analysis
        }
        
        print(f"✅ Enhanced validation complete with {len(case_study_analysis.get('similar_cases', []))} historical references")
...
# Attack Storage API Endpoints
# These endpoints handle attack data persistence for the frontend

@app.post("/api/attacks")
async def save_attack(attack_data: Dict[str, Any]):
    """
    Save attack configuration and results to storage
    Expected by frontend attackStorage.ts
    """
    try:
        print(f"💾 Saving attack data: {attack_data.get('configured_attack', {}).get('attack_id', 'unknown')}")
        
        # For now, return a mock success response
        # In production, this would save to a real database
        attack_session_id = f"attack_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
@app.get("/api/attacks")
async def get_attacks(user_id: str = "sample_user_123", limit: int = 50):
    """
    Retrieve attack history for a user
    Expected by frontend attackStorage.ts
    """
    try:
        print(f"📋 Retrieving attacks for user: {user_id}")
...
if __name__ == "__main__":
    ...
    print(f"📖 Docs available at: http://localhost:8080/docs")
